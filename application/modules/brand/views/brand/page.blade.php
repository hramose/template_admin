...
@section('title') PBM - {{lang('brand')}} @stop
...
<div class="page-content">
    ...
    <!-- END PAGE BAR -->
    <!-- BEGIN PAGE TITLE-->
    <h3 class="page-title"> {{lang('brand')}} </h3>
    <!-- END PAGE TITLE-->
    <!-- END PAGE HEADERs-->
    <div class="row">
        <div class="col-md-12">
            ...
            <div id="table-wrapper" class="portlet light bordered">
                <div class="portlet-title">
                    <div class="caption font-dark">
                        ...
                        <span class="caption-subject">{{lang('brand')}}</span>
                    </div>
                    <div class="tools">
                        @if($add_access == 1)
                            <button onclick="add_brand()" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i>{{lang('new_brand')}}</button>
                        @endif

                        @if($print_limited_access == 1 || $print_unlimited_access == 1)
                            <button onClick="return window.open('{{base_url()}}master/brand/pdf')" class="btn btn-danger btn-sm">
                                <i class="fa fa-file-pdf-o"></i> Print PDF
                            </button>
                            <button onClick="return window.open('{{base_url()}}master/brand/excel')" class="btn btn-success btn-sm">
                ...
                <div class="portlet-body">
                    <table id="table-brand" class="table table-striped table-bordered table-hover dt-responsive" width="100%" >
                        <thead>
                            <tr>
  ...
  <div class="modal-dialog" style="width:50%;">
    <div class="modal-content">
      ...
      <div class="modal-body">
        {{ form_open(null,array('id' => 'form-brand', 'class' => 'form-horizontal', 'autocomplete' => 'off')) }}
...
<script type="text/javascript">
    ...
    var oTable =$('#table-brand').dataTable({
        ...
        "bLengthChange": true,
        "sServerMethod": "GET",
        "sAjaxSource": "{{ base_url() }}brand/brands/fetch_data",
        "columnDefs": [
    ...
    var form_validator = $("#form-brand").validate({
        ...
        messages: {
            brand_name: "{{lang('brand_name')}}" + " {{lang('not_empty')}}",
        },
        submitHandler : function(form){
            ...
            $(form).ajaxSubmit({  
                ...
                url:       '{{base_url()}}brand/brands/save',      
                type:      'POST',       
                clearForm: true ,       
                resetForm: true ,  
            }); 
    ...
    function viewData(value){   
        ...
        $.getJSON('{{base_url()}}brand/brands/view', {id: value}, function(json, textStatus) {
            if(json.status == "success"){
                var row = json.data;
                $('[name="rowID"]').val(row.rowID);
                $('[name="brand_name"]').val(row.brand_name);
    ...
    function deleteData(value){
        ...
        $.confirm({
            ...
            confirm: function() {
                ...
                $.getJSON('{{base_url()}}brand/brands/delete', {id: value}, function(json, textStatus) {
                    if(json.status == "success"){
